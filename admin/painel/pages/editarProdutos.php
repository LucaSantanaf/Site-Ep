...
<table class="list" border="2">
  ...
  <?php
    ...
    if($selecionar_produtos->rowCount() == 0){
      echo '<tr><td colspan="5">Não foram encontrados produtos no Banco de Dados</td></tr>';
    }
    else{
...
<div class="pagination" id="pagina">
  <?php
  	...
  	for($i = $pg-$links; $i<=$pg-1;$i++){
  		if($i<=0){}else{
  			echo '<a href="Index.php?pagina=editarProdutos&pg='.$i.'">'.$i.'</a>';
  		}
  	}
    echo '<strong>'.$pg.'</strong>';
  	...
  	echo '<a href="Index.php?pagina=editarProdutos&pg='.$pags.'">Última Página</a>';
  ?>
</div>
</body>
